...
export class CardsGenerator {
    ...
    public generateMainTextures(renderer: Renderer): Texture[] {
        console.debug(`[AssetService] Generating '${this.cfg.CARDS_TEMPLATE_COUNT}' card textures.`);

        const cardTextures: Texture[] = [];
        for (let i = 0; i < this.cfg.CARDS_TEMPLATE_COUNT; i++) {
    ...
    private drawVoronoiPattern(graphics: Graphics, points: any[], width: number, height: number): void {
        graphics.rect(0, 0, width, height).fill({ color: this.cfg.VORONOI_TINTED_COLOR }); // recommend deep charcoal, good for tinting
    ...
    /**
     * Bakes the <b>GameConfig.CARDS.CARDS_TOTAL_COUNT</b> stack textures by layering the outline and cellular
